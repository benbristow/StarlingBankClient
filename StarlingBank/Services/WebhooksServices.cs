...
public class WebhooksServices : IWebhooksServices
{
    ...
    public async Task DispatchWebhookAsync(StarlingClient starlingClient,
        Models.DefaultWebhookPayloadModel defaultWebhookPayloadModel = null)
    {
        ...
        if (response.StatusCode == HttpStatusCode.InternalServerError)
            throw new APIException(
                "This, and all other HTTP codes, will be treated as a failure of hook receipt and Starling will resend with exponential back-off",
                request,
                response);
        //handle errors defined at the API level
        await _baseServices.ValidateResponse(request, response);
