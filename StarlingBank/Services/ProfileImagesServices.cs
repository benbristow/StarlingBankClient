...
public class ProfileImagesServices : IProfileImagesServices
{
    ...
    public async Task<dynamic> DownloadProfileImageAsync(StarlingClient starlingClient, Guid accountHolderUid)
    {
        ...
        APIHelper.AppendUrlWithTemplateParameters(queryBuilder,
            new Dictionary<string, object>() { { "accountHolderUid", accountHolderUid } });
    ...
    public async Task UpdateProfileImageAsync(StarlingClient starlingClient, Guid accountHolderUid, string contentType,
        object inputStream)
    {
        //prepare query string for API call
        var queryBuilder = new StringBuilder();
        ...
        {
            { "user-agent", "Starling Bank C# Client" }, { "Content-Type", contentType },
            { "Authorization", $"Bearer {starlingClient.OAuthAccessToken}" }
        };
